...
public class BoardComponent extends JPanel {
    ...
    public BoardComponent() {
        ...
        Image retsicon = new ImageIcon("src\\view\\pictures\\returnicon.jpg").getImage().getScaledInstance(65, 65,
                Image.SCALE_DEFAULT);
        ...
        Image cheatt = new ImageIcon("src\\view\\pictures\\cheaticon.png").getImage().getScaledInstance(100, 100,
                Image.SCALE_DEFAULT);
    ...
    public void setMapData() {
        if (playerNum + AINum == 1) {
            ...
            String MapData = "当前玩家:" + suMembers[board.getPlayerNow()].getName(board.getPlayerNow()) + " 剩余雷数:"
                    + board.getRemain();
            ...
            mapData.setFont(font);
            String MapData = "当前玩家:" + suMembers[board.getPlayerNow()].getName(board.getPlayerNow()) + " 剩余雷数:"
                    + board.getRemain() + " 剩余行动数:" + (moveNum - board.getMove());
            mapData.setText(MapData);
        }
    }
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        if (e.getButton() == MouseEvent.BUTTON1) {
                            ...
                            if (playerNum + AINum == 1) {
                                ...
                                String MapData = "当前玩家:" + suMembers[board.getPlayerNow()].getName(board.getPlayerNow())
                                        + " 剩余雷数:" + board.getRemain() + " 剩余行动数:" + (moveNum - board.getMove());
                                mapData.setText(MapData);
                            }
                            String memberData = suMembers[board.getPlayerNow()].getName(board.getPlayerNow()) + " 得分数:"
                                    + board.getPlayers()[board.getPlayerNow()].getScore() + "失误数:"
                                    + board.getPlayers()[board.getPlayerNow()].getMiss();
                            suMembers[board.getPlayerNow()].scoreBoard.setText(memberData);
                            /*
                             * if(board.isend){ int max=0; for(int i=1;i<=playerNum+AINum;i++){
                             * if(board.getPlayers()[i].getScore()>board.getPlayers()[max].getScore()){
                             * max=i; } else
                             * if(board.getPlayers()[i].getScore()==board.getPlayers()[max].getScore()){
                             * if(board.getPlayers()[i].getMiss()==board.getPlayers()[max].getMiss()); } }
                             * JOptionPane.showMessageDialog(gameBoard, "", "游戏结束",
                             * JOptionPane.INFORMATION_MESSAGE, null); } //TODO:结束判断,并补充插旗结束
                             */
                        }
                        ...
                        if (used != now) {
                            if (used == 1) {
                                Image Picture0 = new ImageIcon("src\\view\\individuation\\member01Dark.jpg").getImage()
                                ...
                                Image Picture1 = new ImageIcon("src\\view\\individuation\\member02.jpg").getImage()
                                        .getScaledInstance(160, 160, Image.SCALE_DEFAULT);
                                ImageIcon picture1 = new ImageIcon(Picture1);
                                suMembers[now].setMemberImage(picture1);
                            } else if (used == 2) {
                                if (now == 1) {
                                    Image Picture0 = new ImageIcon("src\\view\\individuation\\member02Dark.jpg")
                                            .getImage().getScaledInstance(160, 160, Image.SCALE_DEFAULT);
                                    ImageIcon picture0 = new ImageIcon(Picture0);
                                    suMembers[used].setMemberImage(picture0);
                                    Image Picture1 = new ImageIcon("src\\view\\individuation\\member01.jpg").getImage()
                                            .getScaledInstance(160, 160, Image.SCALE_DEFAULT);
                                    ImageIcon picture1 = new ImageIcon(Picture1);
                                    suMembers[now].setMemberImage(picture1);
                                } else {
                                    ...
                                    ImageIcon picture0 = new ImageIcon(Picture0);
                                    suMembers[used].setMemberImage(picture0);
                                    Image Picture1 = new ImageIcon("src\\view\\individuation\\member03.jpg").getImage()
                                            .getScaledInstance(160, 160, Image.SCALE_DEFAULT);
                                    ImageIcon picture1 = new ImageIcon(Picture1);
                                    suMembers[now].setMemberImage(picture1);
                                }
                                ...
                                if (now == 1) {
                                    Image Picture0 = new ImageIcon("src\\view\\individuation\\member03Dark.jpg")
                                            .getImage().getScaledInstance(160, 160, Image.SCALE_DEFAULT);
                                    ImageIcon picture0 = new ImageIcon(Picture0);
                                    suMembers[used].setMemberImage(picture0);
                                    Image Picture1 = new ImageIcon("src\\view\\individuation\\member01.jpg").getImage()
                                    ...
                                    ImageIcon picture0 = new ImageIcon(Picture0);
                                    suMembers[used].setMemberImage(picture0);
                                    Image Picture1 = new ImageIcon("src\\view\\individuation\\member04.jpg").getImage()
                                            .getScaledInstance(160, 160, Image.SCALE_DEFAULT);
                                    ImageIcon picture1 = new ImageIcon(Picture1);
                                    suMembers[now].setMemberImage(picture1);
                                }
                            } else if (used == 4) {
                                Image Picture0 = new ImageIcon("src\\view\\individuation\\member04Dark.jpg").getImage()
                                        .getScaledInstance(160, 160, Image.SCALE_DEFAULT);
                                ImageIcon picture0 = new ImageIcon(Picture0);
                                suMembers[used].setMemberImage(picture0);
                                Image Picture1 = new ImageIcon("src\\view\\individuation\\member01.jpg").getImage()
        ...
        cheat.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (board.hadinit) {
                    for (int x = 1; x <= rows; x++) {
                        for (int y = 1; y <= cols; y++) {
                            Image cheatmine = new ImageIcon("src\\view\\pictures\\mineCheat.jpg").getImage()
                                    .getScaledInstance(25, 25, Image.SCALE_DEFAULT);
                            ImageIcon cheatmineicon = new ImageIcon(cheatmine);
                            if (board.getMineState(x, y) == 9)
                                buttons[x][y].setIcon(cheatmineicon);
                        }
                    }
                } else {
                    board.boardInit(1, 1);
